          ...
          <div ref={gridRef}>
            {!history ? (
                ...
                <div className="fancy-grid-wrapper">
                  {d?.groups.map((g, n) => (
                    <div
                      ...
                      className="inventory-overview-card-wrapper"
                      style={elements[n]}>
                      <IonCard className="inventory-overview-card">
                        <IonCardHeader>
                          <IonCardTitle>
                            {"state" in g
                        ...
                        {"category" in g && (g.category.accessories?.length ?? 0) > 0 && (
                          <IonCardContent className="inventory-row">
                            {g.category.accessories
                              ?.filter(
                                i => Capacitor.isNativePlatform() || i.link.startsWith("http")
                              )
                              .map(i => (
                                <IonButton
                                  key={i.label}
              ...
              <div className="fancy-grid-wrapper">
                {d?.history.map((g, n) => (
                  <div
                    key={`card_${g.title[0]}_${stringify(g.title[1])}_${g.time.format()}_${g.types.map(i=>i.icon ?? i.name ?? i.type?.name).join(",")}`}
                    className="inventory-overview-card-wrapper"
                    style={elements[n]}>
                    <IonCard className="inventory-history-card">
                      <IonCardHeader>
